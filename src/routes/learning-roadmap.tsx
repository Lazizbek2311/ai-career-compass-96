...
import { CheckCircle2, Circle, PlayCircle, Clock, ExternalLink, Trophy } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-3xl p-6 sm:p-8 shadow-elegant">
          <Badge className="rounded-full">{t("roadmap.personalizedFor")} {career}</Badge>
          <h1 className="mt-3 text-3xl sm:text-4xl font-bold tracking-tight">{t("roadmap.heading")}</h1>
          <p className="mt-2 text-muted-foreground max-w-2xl">{t("roadmap.subtitle")}</p>
          <div className="mt-5 flex items-center gap-4">
            <Progress value={overall} className="h-2 flex-1" />
            <span className="text-sm font-semibold">{overall}%</span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">{completed} {t("roadmap.of")} {total} {t("roadmap.milestones")}</p>
        </motion.div>
            ...
            <motion.div key={phase} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: pi * 0.05 }} className="glass rounded-3xl p-6 shadow-elegant">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold">{t(`roadmap.${phase}`)} {t("roadmap.phase")}</h2>
                    <p className="text-xs text-muted-foreground">{items.reduce((s, c) => s + c.hours, 0)} {t("roadmap.hoursEstimated")}</p>
                  </div>
                </div>
                <Badge variant="secondary" className="rounded-full">{pPct}% {t("roadmap.complete")}</Badge>
              </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
                            ...
                            <a href={c.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary hover:underline">
                              <ExternalLink className="h-3 w-3" /> {t("roadmap.resource")}
                            </a>
                          </div>
                        </div>
                        <Button size="sm" variant={isDone ? "secondary" : "default"} className="rounded-full" onClick={() => toggle(c.id)}>
                          {isDone ? t("roadmap.done") : <><PlayCircle className="h-3.5 w-3.5 mr-1" />{t("roadmap.continue")}</>}
                        </Button>
                      </div>
                    </div>
                  );
